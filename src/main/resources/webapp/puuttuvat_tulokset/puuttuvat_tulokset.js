...
function displayHakuListResponse(responseText) {
  ...
  parsedResponse.forEach(function (row) {
    ...
    var hakuRowText =
      'Haku ' +
      row.hakuOid +
      ' : myöhäisin koulutuksen alkamiskausi ' +
      row.myohaisinKoulutuksenAlkamiskausi +
      ' , hakukohteita kaikkiaan ' +
      row.hakukohteidenLkm +
      ' , ' +
      (hakuOnTarkistettu
        ? 'puuttuvat tarkistettu ' + row.tarkistettu
        : 'puuttuvia ei ole tarkistettu') +
      (hakuOnTarkistettu
...
function updateMissingResultsForAllHakus() {
  var paivitaMyosOlemassaolevat = document.getElementById(
    'paivitaMyosOlemassaolevat'
  ).checked
  var request = {
    method: 'post',
    body: JSON.stringify({
      paivitaMyosOlemassaolevat: paivitaMyosOlemassaolevat,
    }),
    headers: new Headers({ 'Content-Type': 'application/json' }),
  }
  var statusSuffix = paivitaMyosOlemassaolevat
    ? ' tietojen päivitys puuttuvista tuloksista kaikille hauille.'
  ...
  puuttuvatFetch('/valinta-tulos-service/auth/puuttuvat/paivitaKaikki', request)
    ...
    .then(function (responseText) {
      ...
      if (taustapaivityksenTila.kaynnistettiin) {
        ...
        showStatus(
          'Ei käynnistetty taustapäivitystä, koska se on jo käynnistetty ' +
...
function loadHakukohdeSpecificDataFor(hakuOid, hakuRowElement) {
  showAjaxIndicator()
  puuttuvatFetch('/valinta-tulos-service/auth/puuttuvat/haku/' + hakuOid, {
    method: 'get',
  })
    .then(handleResponse)
...
function createTarjoajaElement(tarjoajaRow) {
  ...
  tarjoajaElement.appendChild(
    document.createTextNode(
      tarjoajaRow.tarjoajanNimi + ' (' + tarjoajaRow.tarjoajaOid + ')'
    )
  )
  ...
  tarjoajaRow.puuttuvatTulokset.forEach(function (hakukohdeRow) {
    ...
    var puuttuvaTulosText =
      hakukohdeRow.puuttuvienMaara === 1
        ? '1 puuttuva tulos'
        : hakukohdeRow.puuttuvienMaara + ' puuttuvaa tulosta'
    linkToSijoittelunTuloksetElement.appendChild(
      document.createTextNode(
        hakukohdeRow.kohteenNimi +
          ' (' +
          hakukohdeRow.hakukohdeOid +
          '), ' +
          puuttuvaTulosText
      )
    )
    ...
    puuttuvatFetch('/valinta-tulos-service/auth/puuttuvat/', request)
      ...
      .then(function (responseText) {
        ...
        if (taustapaivityksenTila.kaynnistettiin) {
          ...
          showStatus(
            'Ei käynnistetty taustapäivitystä haulle ' +
              hakuOid +
              ', koska se on jo käynnistetty ' +
              taustapaivityksenTila.kaynnistetty +
              ' ' +
              taustapaivityksenTila.hakujenMaara +
              ' haulle.'
      ...
      .catch(handleResponse)
    wrapper.appendChild(
      document.createTextNode('Käynnistettiin päivitys ' + new Date())
    )
  }
...
function toggleShowingHakusWithNoMissingData() {
  var rowsOfHakusThatAreOk = document.getElementsByClassName(
    'ei-loydy-puuttuvia'
  )
