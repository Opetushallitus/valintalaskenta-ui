        ...
        return {
          request: function (config) {
            config.headers['Caller-Id'] =
              '1.2.246.562.10.00000000001.valintalaskenta-ui.frontend.jononhallinta'
  ...
  .controller('DashboardController', [
    ...
    function (
      ...
      $scope.isShown = function (elem, idx, arr) {
        return (
          $scope.stateFilters[elem.tila] ||
          _.every($scope.stateFilters, function (state) {
            return !state
          })
        )
      }
      ...
      $scope.showRemoveOption = function (job) {
        ...
        return (
          job.tila === JOB_STATES.QUEUEING &&
          (job.userOID === me.oid ||
            _.isEmpty(
              _.filter(me.roles, function (x) {
                return _.includes(x, '1.2.246.562.10.00000000001')
              })
            ))
        )
      }
      ...
      var updateJobList = function () {
        ...
        seurantaservice.getJobs().then(function (jobs) {
          ...
          $scope.jobsByState = {
            running: _.size(
              _.filter($scope.jobs, function (job) {
                return job.tila == JOB_STATES.RUNNING
              })
            ),
            cancelled: _.size(
              _.filter($scope.jobs, function (job) {
                return job.tila == JOB_STATES.CANCELLED
              })
            ),
            queueing: _.size(
              _.filter($scope.jobs, function (job) {
            ...
            completed: _.size(
              _.filter($scope.jobs, function (job) {
                return job.tila == JOB_STATES.COMPLETED
              })
            ),
          }
        })
      ...
      var queryUserByOid = function (job, userOID) {
        ...
        if ($scope.usernamesByOid[userOID]) {
          ...
          seurantaservice.queryUsernameByOid(userOID).then(
            function (res) {
              if (res.errorCode === 401) {
                job.userNameInitials = ''
                job.userFullname = userOID
                console.log(
                  'Remembering unauthorized name information for oid ' +
                    userOID +
                    'as empty, no further ONR requests for this.'
                )
                $scope.usernamesByOid[userOID] = {
                  initials: '',
                  fullName: userOID,
                }
              } else if (res.etunimet && res.sukunimi) {
                var nameInfo = {
                  initials: _.head(res.etunimet) + _.head(res.sukunimi),
                  fullName: res.etunimet + ' ' + res.sukunimi,
                }
                job.userNameInitials = nameInfo.initials
                job.userFullname = nameInfo.fullName
                console.log(
                  'Remembering name information for oid ' + userOID + ' as ',
                  nameInfo
                )
                $scope.usernamesByOid[userOID] = nameInfo
              } else {
                console.log(
                  'Name information not available for user, using empty default initials. ' +
                    userOID
                )
              }
            },
            function (err) {
              console.error(
                'Unhandled error querying username, using empty default initials. ',
                err
              )
            }
          )
