angular
  ...
  .factory('VastaanottoUtil', [
    'VastaanottoAikarajanMennytTieto',
    '$q',
      ...
      return {
        merkitseMyohastyneeksi: function (valintatulokset) {
          ...
          valintatulokset.forEach(function (valintatulos) {
            if (
              valintatulos.julkaistavissa &&
              'HYLATTY' !== valintatulos.tila &&
              ('EI_VASTAANOTETTU_MAARA_AIKANA' === valintatulos.tilaHakijalle ||
                valintatulos.vastaanottoAikarajaMennyt)
            ) {
        ...
        fetchAndPopulateVastaanottoDeadlineDetailsAsynchronously: function (
          hakuOid,
          hakukohdeOid,
          kaikkiHakemukset,
          ...
          aikarajaMennytDeferred.promise.then(function (aikarajaMennytTiedot) {
            _.forEach(aikarajaMennytTiedot, function (
              vastaanottoAikarajaMennyt
            ) {
              _.forEach(kaikkiHakemukset, function (hakemus) {
                if (
                  hakemus &&
                  hakemus.hakemusOid === vastaanottoAikarajaMennyt.hakemusOid
                ) {
                  hakemus.vastaanottoAikarajaMennyt =
                    vastaanottoAikarajaMennyt.mennyt
                  hakemus.vastaanottoAikaraja =
                    vastaanottoAikarajaMennyt.vastaanottoDeadline
                }
              })
            })
          ...
          if (
            oiditHakemuksilleJotkaTarvitsevatAikarajaMennytTiedon.length > 0
          ) {
...
app.factory('HakukohteenValintatuloksetIlmanTilaHakijalleTietoa', function (
  $resource
) {
...
app.factory('ValinnanTulos', function ($http) {
  ...
  return {
    get: function (params, config) {
      return $http.get(
        plainUrls.url('valinta-tulos-service.valinnan-tulos.haku', params),
        config || {}
      )
    },
    patch: function (valintatapajonoOid, data, config) {
      return $http.patch(
        plainUrls.url(
