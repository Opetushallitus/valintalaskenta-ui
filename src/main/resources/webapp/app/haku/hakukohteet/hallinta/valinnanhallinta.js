...
app.factory('ValinnanhallintaModel', function (
  ...
  model = new (function () {
    ...
    this.refresh = function (hakukohdeOid) {
      if (hakukohdeOid !== undefined) {
        ...
        ValinnanvaiheListFromValintaperusteet.get(
          { hakukohdeoid: hakukohdeOid },
          function (result) {
            model.tulosValinnanvaiheet = result.map((valinnanvaihe) =>
              Object.assign({}, valinnanvaihe, {
                jonot: valinnanvaihe.jonot
                  .map((valintatapajono) =>
                    Object.assign(
                      {},
                      valintatapajono,
                      valintatapajono.eiLasketaPaivamaaranJalkeen
                        ? {
                            eiLasketaPaivamaaranJalkeen: new Date(
                              valintatapajono.eiLasketaPaivamaaranJalkeen
                            ),
                          }
                        : undefined
                  ...
                  .filter((valintatapajono) => valintatapajono.aktiivinen)
                  .sort((j, jj) => j.prioriteetti - jj.prioriteetti),
              })
            )

            var anyHasValisijoittelu = !_.every(result, {
              hasValisijoittelu: false,
            })
...
angular.module('valintalaskenta').controller('ValinnanhallintaController', [
  '$scope',
  '$routeParams',
  '$modal',
  'Latausikkuna',
  'Ilmoitus',
  'ValinnanhallintaModel',
  'HakukohdeModel',
  'ParametriService',
  'IlmoitusTila',
  'HakuModel',
  function (
    ...
    $scope.kaikkiKerralla = function () {
      ...
      var nimi =
        ($scope.hakukohdeModel.tarjoajaNimi
          ? $scope.hakukohdeModel.tarjoajaNimi + ': '
          : '') + $scope.hakukohdeModel.hakukohdeNimi
    ...
    $scope.valintatapajonoEiLaskettavissa = function (valintatapajono) {
      return (
        valintatapajono.eiLasketaPaivamaaranJalkeen &&
        valintatapajono.eiLasketaPaivamaaranJalkeen < new Date()
      )
    }
  },
])
