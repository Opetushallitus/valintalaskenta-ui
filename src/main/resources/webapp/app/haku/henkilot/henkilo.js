...
app.factory('HenkiloModel', function (
  $resource,
  $q,
  $routeParams,
  Henkilot,
  ...
  function getAtaruHakemukset(hakuOid, start, n, q) {
    return AtaruApplications.get({ hakuOid: hakuOid, name: q }).$promise.then(
      function (hakemukset) {
        return {
  ...
  function getHakemukset(hakuOid, start, n, q) {
    return HakuModel.refreshIfNeeded(hakuOid)
      .then(function (haku) {
  ...
  function getNextPage() {
    ...
    if (this.readyToQueryForNextPage) {
      ...
      getHakemukset(
        this.hakuOid,
        this.hakemukset.length,
        this.pageSize,
        $.trim(this.searchWord)
      ).then(function (result) {
...
angular.module('valintalaskenta').controller('HenkiloController', [
  '$scope',
  '$location',
  '$routeParams',
  '$timeout',
