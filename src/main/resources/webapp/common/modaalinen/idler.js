...
app.directive('idle', [
  ...
  '$timeout',
  '$interval',
  function (Idle, $timeout, $interval) {
    return {
      restrict: 'A',
...
app.controller('SessionExpiresCtrl', [
  'Idle',
  '$scope',
  '$modalInstance',
  '$window',
  'LocalisationService',
  function (Idle, $scope, $modalInstance, $window, LocalisationService) {
    $scope.timeoutMessage = function () {
      var duration = Math.floor(MAX_SESSION_IDLE_TIME_IN_SECONDS / 60)
      return (
        LocalisationService.tl('session.expired.text1.part1') +
        ' ' +
        duration +
        ' ' +
        LocalisationService.tl('session.expired.text1.part2')
      )
    }
...
app
  .controller('EventsCtrl', [
    ...
    '$modal',
    '$http',
    function ($scope, Idle, $modal, $http) {
      ...
      $scope.$on('IdleWarn', function (e, countdown) {
        if (
          !$scope.sessionWarning ||
          angular.element('#sessionWarning').length < 1
        ) {
