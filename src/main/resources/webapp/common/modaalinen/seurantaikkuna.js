function SeurantaIkkunaCtrl(
  ...
  ValintalaskentaKerrallaAktivointi,
  Ilmoitus,
  IlmoitusTila,
  SeurantaPalvelu,
  ...
  $scope.isKaynnissa = function () {
    // onko ajossa tai onko mielekasta enaa
    // ajaakkaan
  ...
  function laskennanVastaus(uuid) {
    ...
    if (uuid.lisatiedot) {
      $scope.seurataankoOlemassaAjossaOlevaaLaskentaa = !uuid.lisatiedot
  ...
  $scope.getProsentit = function () {
    if ($scope.kaikkityot == 0) {
      ...
      return Math.round(
        (($scope.tehty + $scope.ohitettu) / $scope.kaikkityot) * 100
      )
    }
  }
}
