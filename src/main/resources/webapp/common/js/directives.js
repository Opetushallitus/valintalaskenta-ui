...
app.directive('resolveNamesFromTarjonta', function ($http, $q) {
        ...
        .then(
          function (result) {
            if (
              ...
              result.data &&
              result.data.result &&
              result.data.result.hakukohteenNimet
            ) {
  ...
  function link(scope, element, attrs) {
    if (attrs.message) {
      ...
      var hakukohdeOid =
        hakukohdeMatch && hakukohdeMatch.length > 1 ? hakukohdeMatch[1] : null
...
app.directive('centralize', function () {
  return {
    ...
    link: function ($scope, element, attrs) {
      ...
      var top = Math.max(
        0,
        ($(window).height() - elemHeight) / 2 + $(window).scrollTop()
      )
      var left = Math.max(
        0,
        ($(window).width() - elemWidth) / 2 + $(window).scrollLeft()
      )
      $(element).css({
        margin: 0,
        top: (top > 0 ? top : 0) + 'px',
        ...
        if (FLOAT_REGEXP.test(viewValue)) {
          ...
          if (!isNaN(min) && !isNaN(max) && floatVal) {
            if (min <= floatVal && max >= floatVal) {
              ...
              $(elm)
                .siblings('span')
                .text('Arvo ei ole välillä ' + min + ' - ' + max)
                ...
                $scope.update = function () {
                  if ($scope.hakemus) {
                    ...
                    var valinnantilanMuutos = [
                      {
                        ...
                        ehdollisestiHyvaksyttavissa:
                          h.ehdollisestiHyvaksyttavissa,
                        ehdollisenHyvaksymisenEhtoKoodi:
                          h.ehdollisenHyvaksymisenEhtoKoodi,
                        ehdollisenHyvaksymisenEhtoFI:
                          h.ehdollisenHyvaksymisenEhtoFI,
                        ehdollisenHyvaksymisenEhtoSV:
                          h.ehdollisenHyvaksymisenEhtoSV,
                        ehdollisenHyvaksymisenEhtoEN:
                          h.ehdollisenHyvaksymisenEhtoEN,
                    ...
                    ValinnanTulos.patch(
                      ...
                      {
                        headers: {
                          'X-If-Unmodified-Since': $scope.lastmodified,
                        },
                      }
                    ).then(
                      function (result) {
                        ...
                        Ilmoitus.avaa(
                          'Tallennus onnistui',
                          'Sijoittelun vastaanottotila muutettu.'
                        )
                      },
                      function (error) {
                        $scope.error = error
          ...
          .open({
            ...
            controller: function ($scope, $window, $modalInstance, Ilmoitus) {
              $scope.update = function () {
                var postParams = {
                  ...
                  harkinnanvaraisuusTila:
                    $scope.hakemus.muokattuHarkinnanvaraisuusTila,
                }
              ...
              var setTila = function (params) {
                HarkinnanvaraisestiHyvaksytty.get(
                  params,
                  function (result) {
                    result.forEach(function (harkinnanvarainen) {
                      if (
                        $scope.hakukohdeOid == harkinnanvarainen.hakukohdeOid
                      ) {
                        $scope.hakemus.muokattuHarkinnanvaraisuusTila =
                          harkinnanvarainen.harkinnanvaraisuusTila
                        $scope.hakemus.harkinnanvaraisuusTila =
                          harkinnanvarainen.harkinnanvaraisuusTila
                      }
                    })
                    ...
                    Ilmoitus.avaa(
                      'Tallennus onnistui',
                      'Harkinnavaraisuuden tila muutettu.'
                    )
                  },
                  function (error) {
                    $scope.error = error
            ...
            .open({
              ...
              controller: function ($scope, $window, $modalInstance, Ilmoitus) {
                ...
                $scope.jonosija.muokkaus.arvo =
                  $scope.jonosija.jarjestyskriteerit[0].arvo
                if ($scope.jonosija.jarjestyskriteerit[0].kuvaus.FI) {
                  $scope.jonosija.muokkaus.selite =
                    $scope.jonosija.jarjestyskriteerit[0].kuvaus.FI
                }
                ...
                $scope.update = function () {
                  var updateParams = {
                    ...
                    jarjestyskriteeriprioriteetti:
                      $scope.jonosija.muokkaus.jarjestyskriteeriPrioriteetti
                        .value,
                  }
                  ...
                  JarjestyskriteeriMuokattuJonosija.post(
                    updateParams,
                    postParams,
                    function (result) {
                      $modalInstance.close(result)
                      Ilmoitus.avaa(
                        'Tallennus onnistui',
                        'Järjestyskriteerin tila muutettu.'
                      )
                ...
                $scope.remove = function () {
                  ...
                  JarjestyskriteeriMuokattuJonosija.remove(
                    updateParams,
                    function (result) {
                      $modalInstance.close(result)
                      Ilmoitus.avaa(
                        'Poisto onnistui',
                        'Muokattu järjestyskritteri on poistettu.'
                      )
                      $route.reload()
                    },
                    function (error) {
...
app.directive('valintatulos', function ($rootScope) {
  var resultState = {
    ...
    VarallaPvm: {
      FI:
        '__varasija__._varasijalla. Varasijoja täytetään __varasijaPvm__ asti.',
      SV:
        'På __varasija__._reservplats. Reservplatser besätts fram till __varasijaPvm__.',
    },
    Peruutettu: {
      FI: 'Peruutettu',
      SV: 'Annullerad',
  ...
  function resultStateToText(value) {
    ...
    return translations ? translations[language] || translations['FI'] : value
  }

  function isHyvaksyttyKesken(valintatulos, valintatulokset) {
    if (valintatulos.valintatila === 'HYVAKSYTTY') {
      var firstKeskenIndex = valintatulokset.findIndex(function (v) {
        return v.valintatila === 'KESKEN' || v.valintatila === 'VARALLA'
      })
      if (firstKeskenIndex != -1) {
        var valintatulosIndex = valintatulokset.findIndex(function (v) {
          return v.hakukohdeOid === valintatulos.hakukohdeOid
        })
        // If hyvaksytty hakemus is not vastaanotettavissa it belongs to kk haku with sijoittelu
        return (
          firstKeskenIndex < valintatulosIndex &&
          valintatulos.vastaanotettavuustila === 'EI_VASTAANOTETTAVISSA'
        )
      }
    }
    return false
  ...
  function underscoreToCamelCase(str) {
    return str
      .toLowerCase()
      .replace(/^(.)|_(.)/g, function (match, char1, char2) {
  ...
  return {
    ...
    link: function ($scope, element) {
      $scope.formatDate = function (dt) {
        ...
        else return moment(dt).format('D.M.YYYY ').replace(/,/g, '')
      }

      $scope.$watch('isFinal()', function (value) {
        $scope.status = value
          ? 'personinformationmodal.lopullinen'
          : 'personinformationmodal.kesken'
      })

      $scope.valintatulosText = function (valintatulos, valintatulokset) {
        var key = isHyvaksyttyKesken(valintatulos, valintatulokset)
          ? 'HyvaksyttyKesken'
          : underscoreToCamelCase(valintatulos.valintatila)
        ...
        if (
          ...
          ].indexOf(valintatulos.vastaanottotila) >= 0
        ) {
          key = underscoreToCamelCase(valintatulos.vastaanottotila)
          return resultStateToText(key)
        } else if (
          !_.isEmpty(valintatulos.tilanKuvaukset) &&
          !_.isEmpty(valintatulos.tilanKuvaukset[lang])
        ) {
          ...
          valintatulos.valintatila === 'VARALLA' &&
          valintatulos.varasijojaTaytetaanAsti != null
        ) {
          return (
            valintatulos.varasijanumero +
            '. varasijalla. Varasijoja täytetään ' +
            $scope.formatDate(valintatulos.varasijojaTaytetaanAsti) +
            ' asti.'
          )
        } else {
      ...
      $scope.valintatulosStyle = function (valintatulos) {
        if (hyvaksytty(valintatulos)) return 'accepted'
      }

      function hyvaksytty(valintatulos) {
        return (
          valintatulos.valintatila == 'HYVAKSYTTY' ||
          valintatulos.valintatila == 'HARKINNANVARAISESTI_HYVAKSYTTY' ||
          valintatulos.valintatila == 'VARASIJALTA_HYVAKSYTTY'
        )
      }
    },
  }
...
app.directive('showPersonInfoWithVtsData', [
  'ValintaTulosProxy',
  function (ValintaTulosProxy) {
      ...
      controller: function ($modal, $scope, LocalisationService) {
        ...
        $scope.show = function () {
          ...
          $modal.open({
            scope: $scope,
            templateUrl:
              '../common/html/personInformationModalWithVTSData.html',
...
app.directive('paginationPagesize', function () {
  return {
    ...
    controller: function ($scope, LocalisationService) {
      $scope.itemsInDropdown = [
        { value: 100000, text: LocalisationService.tl('kaikki') || 'Kaikki' },
      ]
...
app.directive('muokattuVastaanottoTila', function () {
  ...
  function poistaLisahaustaEhdollinenVastaanotto(isLisahaku) {
    return function (tila) {
      return (
        (isLisahaku && tila.value !== 'EHDOLLISESTI_VASTAANOTTANUT') ||
        !isLisahaku
      )
    }
  }
    ...
    controller: function (
      ...
      AuthService.updateOph('APP_VALINTOJENTOTEUTTAMINEN')
        ...
        .then(function (updateOph) {
          $scope.$watchGroup(['haku', 'hakemus'], function (
            newVals,
            oldVals,
            scope
          ) {
  ...
  return {
    ...
    controller: function ($modal, $scope, AuthService, LocalisationService) {
      ...
      var l = function (newval, oldval) {
        if (newval !== oldval) {
          var hyvaksytty =
            'HYVAKSYTTY' === $scope.hakemus.tila ||
            'VARASIJALTA_HYVAKSYTTY' === $scope.hakemus.tila
          var peruuntunut = 'PERUUNTUNUT' === $scope.hakemus.tila

          $scope.showHyvaksyPeruuntunut =
            $scope.onEdit &&
            (($scope.canHyvaksyPeruuntunut && peruuntunut) ||
              ($scope.hakemus.hyvaksyPeruuntunut &&
                (hyvaksytty || peruuntunut)))

          $scope.id =
            $scope.hakemus.valintatapajonoOid +
            '-' +
            $scope.hakemus.hakemusOid.replace(/\./g, '')

          $scope.isHarkinnanvaraisestiHyvaksytty =
            ($scope.hakemus.tila === 'HYVAKSYTTY' ||
              $scope.hakemus.tila === 'VARASIJALTA_HYVAKSYTTY') &&
            $scope.hakemus.hyvaksyttyHarkinnanvaraisesti == true
          ...
          $scope.tilaText =
            hakemuksenTilaToText[$scope.hakemus.tila] || $scope.hakemus.tila
        }
      }
      ...
      AuthService.peruuntuneidenHyvaksyntaOph('APP_SIJOITTELU').then(
        function () {
          $scope.canHyvaksyPeruuntunut = true
          l(true, false)
        },
        function () {
          $scope.canHyvaksyPeruuntunut = false
...
app.directive('pisteidenSyottaminen', function () {
  return {
    ...
    controller: function ($scope, $modal, LocalisationService, R) {
      $scope.t = LocalisationService.tl
      $scope.copyTunnisteValueAcrossHakutoiveet = function (
        tunniste,
        ...
        return function (hakutoive) {
          var hasSomeValue =
            hakutoive.additionalData[tunniste] ||
            hakutoive.additionalData[osallistumisenTunniste]
          if (hasSomeValue) {
      ...
      $scope.changeOsallistuminen = function (
        hakutoiveet,
        hakija,
        tunniste,
        osallistumisenTunniste,
        ...
        if (Array.isArray(hakutoiveet)) {
          R.map(
            $scope.copyTunnisteValueAcrossHakutoiveet(
              tunniste,
              osallistumisenTunniste,
