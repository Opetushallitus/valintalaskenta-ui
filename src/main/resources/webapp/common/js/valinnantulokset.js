...
angular
  ...
  .factory('Valinnantulokset', [
    function () {
      var valinnantulokset = {}

      valinnantulokset.compareOldAndNewVtsResponse = function (
        oldVtsValintatapajono,
        newVtsValinnantulokset,
        checkKeys
      ) {
        compareValintatapajonoValinnantulokset(
          oldVtsValintatapajono,
          newVtsValinnantulokset
        )

        _(oldVtsValintatapajono.hakemukset).forEach(function (oldHakemus) {
          var newValinnantulos = getNewValinnantulos(
            newVtsValinnantulokset,
            oldHakemus
          )
          ...
          if (typeof newValinnantulos === 'undefined') {
            console.log(
              'No valinnantulos for hakemus: ' +
                oldHakemus.hakemusOid +
                ' in new VTS api.'
            )
            splitLog()
          } else {
            ...
            _(checkKeys).forEach(function (key) {
              ...
              if (newValue instanceof Date || oldValue instanceof Date) {
                ...
                if (
                  !(isNaN(newTime) && isNaN(oldTime)) &&
                  newTime !== oldTime
                ) {
                  console.log(
                    'Mismatch with old and new valinnantulos for hakemus: ' +
                      oldHakemus.hakemusOid +
                      ' on key ' +
                      key.oldKey +
                      ' (' +
              ...
              oldHakemus[key.oldKey] = newValue
            })
            if (logValinnantulos) {
      ...
      function getNewValinnantulos(newVtsValinnantulokset, oldHakemus) {
        var newValinnantulokses = _(newVtsValinnantulokset).filter(function (
          newTulos
        ) {
        ...
        if (newValinnantulokses.length > 1) {
          console.log(
            'Too many ' +
              newValinnantulokses.length +
              ' valinnantulos for hakemus ' +
              newValinnantulokses[0].hakemusOid
          )
        } else newValinnantulos = newValinnantulokses[0]
        return newValinnantulos
      }

      var compareValintatapajonoValinnantulokset = function (
        oldVtsValintatapajono,
        newVtsValinnantulokset
      ) {
        ...
        if (hakemukset.length < newVtsValinnantulokset.length) {
          console.log(
            'Too many valinnantulos from VTS for valintatapajono ' +
              oldVtsValintatapajono.oid
          )
          var oldHakemusOids = _(hakemukset).map(function (hakemus) {
              return hakemus.hakemusOid
            }),
            valinnantulosHakemusOids = _(newVtsValinnantulokset).map(function (
              tulos
            ) {
          ...
          console.log(
            'Hakemusoids in VTS response but not in original hakemus list: ' +
              extraOids.join(',')
          )
          splitLog()
        }
      ...
      valinnantulokset.compareErillishakuOldAndNewVtsResponse = function (
        oldVtsValintatapajono,
        newVtsValinnantulokset
      ) {
        valinnantulokset.compareOldAndNewVtsResponse(
          oldVtsValintatapajono,
          newVtsValinnantulokset,
          erillishakuKeys
        )
      ...
      valinnantulokset.compareSijoitteluOldAndNewVtsResponse = function (
        oldVtsValintatapajono,
        newVtsValinnantulokset
      ) {
        valinnantulokset.compareOldAndNewVtsResponse(
      ...
      var erillishakuKeys = [
        // {keyInOld: keyInNew}
        {
          oldKey: 'ehdollisestiHyvaksyttavissa',
          newKey: 'ehdollisestiHyvaksyttavissa',
        },
        ...
        {
          oldKey: 'hyvaksymiskirjeLahetetty',
          newKey: 'hyvaksymiskirjeLahetetty',
        },
      ]
