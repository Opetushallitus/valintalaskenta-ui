function valintakoetulosPage() {
  ...
  var valintakoetuloksetPage = openPage(
    '/valintalaskenta-ui/app/index.html#/haku/HAKU1/hakukohde/HAKUKOHDE1/valintakoetulos',
    function () {
  ...
  var pageFunctions = {
    ...
    findTableWithPanelTitle: function (panelTitle) {
      return S('#kokeittainNakyma div.panel-title:contains(' + panelTitle + ')')
        .parent()
        .parent()
        .find('table') // tr:nth(1) td:nth(1)").text().trim();
    },
    findNthHakijaWithPanelTitle: function (n, panelTitle) {
      return pageFunctions
        .findTableWithPanelTitle(panelTitle)
        .find('tr:nth(' + n + ') td:nth(1)')
        .text()
        .trim()
    },
    ...
    openPage: function (done) {
      return valintakoetuloksetPage().then(
        wait.until(function () {
