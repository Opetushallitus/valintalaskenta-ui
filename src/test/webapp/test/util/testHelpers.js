  ...
  return {
    ...
    isFocusableBefore: function (button) {
      return (
        !this.hasTabIndex() &&
        !button.hasTabIndex() &&
        compareDOMIndex(this.element(), button.element()) < 0
      )
    },
  }
}
...
mockAjax = {
  init: function () {
    ...
    if (testFrame().sinon) deferred.resolve()
    else
    ...
    var fakeAjax = function () {
      ...
      xhr.onCreate = function (request) {
        window.setTimeout(function () {
          if (
            window._fakeAjaxParams &&
            request.method == _fakeAjaxParams.method &&
            request.url == _fakeAjaxParams.url
          ) {
            request.respond(
              _fakeAjaxParams.responseCode,
              { 'Content-Type': 'application/json' },
              _fakeAjaxParams.responseBody
            )
            xhr.restore()
...
function runTestHooks() {
  testHooks.forEach(function (hook) {
    hook()
  })
  testHooks = []
}
  ...
  return seq(
    function () {
      dslDebug(
        ...
        text,
        'item selector:',
        selectItemFn().selector
      )
    },
    visible(inputFn),
    function () {
      return inputFn().val(text).change()
    },
    click(selectItemFn)
  )
}
...
function mockPostReturnData(triggerFn, urlPattern) {
  ...
  return seq(
    function () {
      var savedData
      testFrame()
        .httpBackend.when('POST', urlPattern, function (data) {
...
function assertText(selector, val) {
  chai.assert(
    typeof selector().val() !== 'undefined',
    'element ' + selector().selector + ' should be defined, is undefined'
  )
