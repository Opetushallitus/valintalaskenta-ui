describe('Pistesyöttö', function () {
  ...
  describe('Pistesyöttö, kun koetulostentallennus=false', function () {
    ...
    it(
      'Tallenna nappi on disabloitu',
      seqDone(disabled(pistesyottoselectors.tallennaButton))
    )
  })
  ...
  describe('Pistesyöttö, kun koetulostentallennus=true', function () {
    before(function (done) {
      initCommonFixtures()
      addTestHook(
        parametritFixturesWithOverrides({ koetulostentallennus: true })
      )()
      page.openPage(done)
    })
    describe('Ensimmäisellä sivulla', function () {
      it(
        'Kenttien muokkaus on sallittu',
        seqDone(
          wait.forAngular,
      ...
      it(
        'Tallenna nappi on enabloitu',
        seqDone(enabled(pistesyottoselectors.tallennaButton))
      )
    })
    describe('Toisella sivulla', function () {
      before(seqDone(click(pistesyottoselectors.page2link), wait.forAngular))
      describe('Alkutilassa', function () {
        it(
          'Kenttien muokkaus on sallittu',
          seqDone(enabled(pistesyottoselectors.osallistuminenSelectBox))
        )
        it(
          'Tallenna nappi on enabloitu',
          seqDone(enabled(pistesyottoselectors.tallennaButton))
      ...
      describe('Vaihdettaessa osallistuneeksi', function () {
        ...
        describe('Vaihdon jälkeen', function () {
          it(
            'Pisteitä on mahdollista syöttää',
            seqDone(enabled(pistesyottoselectors.pistearvoInputBox))
          )
        })
      ...
      describe('Vaihdettaessa merkitsemättömäksi', function () {
        before(
          seqDone(
            select(
              pistesyottoselectors.osallistuminenSelectBox,
              'MERKITSEMATTA'
            ),
            wait.forAngular
          )
